...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  if (HAL_ADC_Start_DMA(&hadc1, (uint16_t*)ADCValues, 2) != HAL_OK)
   {
     Error_Handler();
   }
  /* USER CODE END 2 */
  ...
  while (1)
  {
	  ...
	  char potMsg[100];
	      ...
	      /* Evalúa ADCValues[0]: eje vertical */
	      if (ADCValues[0] > 100)
	        strcpy(vertical, "arriba");
	      else if (ADCValues[0] < 200)
	        strcpy(vertical, "abajo");
	      else
	        strcpy(vertical, "centro");

	      /* Evalúa ADCValues[1]: eje horizontal */
	      if (ADCValues[1] > 100)
	        strcpy(horizontal, "derecha");
	      else if (ADCValues[1] < 200)
	        strcpy(horizontal, "izquierda");
	      else
	        strcpy(horizontal, "centro");

	      /* Formatea el mensaje y lo transmite por UART2 */
	      sprintf(potMsg, "jugador 1:   Vertical: %s, Horizontal: %s\r\n", vertical, horizontal);
	      HAL_UART_Transmit(&huart2, (uint8_t*)potMsg, strlen(potMsg), HAL_MAX_DELAY);
	      HAL_Delay(500);
...
/**
  ...
  * Reinicia la conversión para que el DMA actualice ADCValues.
  */
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
  HAL_ADC_Start_DMA(&hadc1, (uint16_t*)ADCValues, 2);
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  if (huart->Instance == USART1) // Verifica si la interrupción viene de USART1
  {
    if (rxByte == '\n')
    {
      ...
      snprintf(Buffer, sizeof(Buffer), "jugador 2 = %s\r\n\r\n", rxBuffer);
